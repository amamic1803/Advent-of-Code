...
impl Day10 {
    ...
    fn find_loop(field: &[Vec<char>]) -> HashSet<(usize, usize)> {
        ...
        let mut current = Self::find_start(field);
        let mut next = Self::find_adjacent_pipes(current, field);
        ...
        while !(visited.contains(&next[0]) && visited.contains(&next[1])) {
            if visited.contains(&next[0]) {
                current = next[1];
                visited.insert(current);
                next = Self::find_adjacent_pipes(current, field);
            } else {
                current = next[0];
                visited.insert(current);
                next = Self::find_adjacent_pipes(current, field);
    ...
    fn determine_s(field: &[Vec<char>]) -> ((usize, usize), char) {
        let s_coord = Self::find_start(field);
...
impl Day for Day10 {
    ...
    fn part1(&self, input: &str) -> Result<String, Error> {
        let field = Self::parse_input(input);
        ...
        Ok((Self::find_loop(&field).len() / 2).to_string())
    }

    fn part2(&self, input: &str) -> Result<String, Error> {
        let mut field = Self::parse_input(input);

        let loop_tiles = Self::find_loop(&field);

        let (s_coord, s_type) = Self::determine_s(&field);
        ...
        let mut expanded_map = Self::expand_loop(&loop_tiles, &field);

        Self::flood_fill_outside(&mut expanded_map);
        ...
        let reduced_map = Self::reduce_loop(&expanded_map);
