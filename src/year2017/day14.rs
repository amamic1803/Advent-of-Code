...
impl Day14 {
    ...
    fn generate_grid(input: &str) -> Vec<Vec<bool>> {
        ...
        let mut grid = Vec::with_capacity(Self::ROWS); // 128x128 grid, true = used, false = free
...
impl Day for Day14 {
    ...
    fn part1(&self, input: &str) -> Result<String, Error> {
        let grid = Self::generate_grid(input.trim());
        let mut count = 0;
    ...
    fn part2(&self, input: &str) -> Result<String, Error> {
        let mut grid = Self::generate_grid(input.trim());
        let mut region_count = 0;

        let mut queue = VecDeque::new();
        while let Some(used_pos) = Self::find_used(&grid) {
            region_count += 1;
            ...
            while !queue.is_empty() {
                ...
                if grid[working_pos.0][working_pos.1] {
                    ...
                    if working_pos.0 < Self::ROWS - 1 && grid[working_pos.0 + 1][working_pos.1] {
                        queue.push_back((working_pos.0 + 1, working_pos.1));
                    }
                    if working_pos.1 > 0 && grid[working_pos.0][working_pos.1 - 1] {
                        queue.push_back((working_pos.0, working_pos.1 - 1));
                    }
                    if working_pos.1 < Self::ROWS - 1 && grid[working_pos.0][working_pos.1 + 1] {
                        queue.push_back((working_pos.0, working_pos.1 + 1));
                    }
                }
            }
