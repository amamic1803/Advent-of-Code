    ...
    fn parse_input(input: &str) -> (Vec<(usize, usize)>, Graph) {
        ...
        for i in 0..numbers_locs.len() {
            for j in (i + 1)..numbers_locs.len() {
                let distance = Self::find_shortest_path(&grid, numbers_locs[i], numbers_locs[j]);
...
impl Day for Day24 {
    ...
    fn part1(&self, input: &str) -> Result<String, Error> {
        ...
        let (numbers_locs, mut graph) = Self::parse_input(input);
    ...
    fn part2(&self, input: &str) -> Result<String, Error> {
        ...
        let (_, graph) = Self::parse_input(input);
