    ...
    fn parse_input(input: &str) -> (Vec<Vec<u32>>, [usize; 2], [usize; 2]) {
        ...
        for (curr_row, line) in input.trim().lines().enumerate() {
            ...
            for (curr_col, x) in line.chars().enumerate() {
                ...
                current_row.push(Self::char_value(x));
            }
...
impl Day for Day12 {
    ...
    fn part1(&self, input: &str) -> Result<String, Error> {
        let (height_map, loc_start, loc_end) = Self::parse_input(input);
        Ok(Self::dijkstra_1(&height_map, loc_start, loc_end).to_string())
    }

    fn part2(&self, input: &str) -> Result<String, Error> {
        let (height_map, _, loc_end) = Self::parse_input(input);
        Ok(Self::dijkstra_2(&height_map, loc_end, 0).to_string())
    }
}
