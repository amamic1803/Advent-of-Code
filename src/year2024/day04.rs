...
impl Day for Day04 {
    ...
    fn part1(&self, input: &str) -> Result<String, Error> {
        ...
        let text = Self::parse_input(input);
        ...
        for (i, row) in text.iter().enumerate() {
            for (j, &c) in row.iter().enumerate() {
                ...
                // E
                if right_padding && Self::check_xmas(|k| row[j + k] == xmas_word[k]) {
                    xmas_count += 1;
                }

                // W
                if left_padding && Self::check_xmas(|k| row[j - k] == xmas_word[k]) {
                    xmas_count += 1;
                }

                // N
                if top_padding && Self::check_xmas(|k| text[i - k][j] == xmas_word[k]) {
                    xmas_count += 1;
                }

                // S
                if bottom_padding && Self::check_xmas(|k| text[i + k][j] == xmas_word[k]) {
                    xmas_count += 1;
                }
